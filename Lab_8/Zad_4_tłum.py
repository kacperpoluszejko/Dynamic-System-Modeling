...
def poincare_map_damped(K, b, theta0, L0_list, N):
    ...
    for j in range(M):
        ...
        for _ in range(N):
            ...
            L = (L_old + K * np.sin(theta_old)) * expb
            theta = theta_old + (L_old + K * np.sin(theta_old)) * coef
...
for K in K_anim:
    theta_points, L_points = poincare_map_damped(K, b, theta0, L0_list, N)

    fig, ax = plt.subplots(figsize=(6,6))
    ax.scatter(theta_points, L_points, s=1)
    ax.set_xlabel(r"$\theta \;(\mathrm{mod}\; 2\pi)$")
    ax.set_ylabel(r"$L$")
    ax.set_title(f"Poincaré (damped), K={K:.2f}, b={b}")
    ax.grid(True)
...
imageio.mimsave("poincare_damped2.gif", frames, duration=0.05)
